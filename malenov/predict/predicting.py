...
from . import cube_parse
from ..segy import csv_struct
...
def predicting(filename, inp_seis, seis_obj, keras_model, cube_incr, num_classes, inp_res=np.float64, mode='xline',
    ...
    # section:       edge locations(index) of the sub-section
    ...
    if show_features:
        ...
        prediction = np.empty((
            (section_edge[5] - section_edge[4] + 1) * (section_edge[3] - section_edge[2] + 1) * (
                section_edge[1] - section_edge[0] + 1), 10),
            dtype=np.float32)
    else:
        prediction = np.empty((
            (section_edge[5] - section_edge[4] + 1) * (section_edge[3] - section_edge[2] + 1) * (
                section_edge[1] - section_edge[0] + 1),
            num_classes), dtype=np.float32)

    # Preallocate the data array to fill for each batch and initiate iterators
    data = np.empty(
        (pred_batch * (section_edge[5] - section_edge[4] + 1), cube_size, cube_size, cube_size, num_channels),
        dtype=inp_res)
    indx = 0
    jndx = 0
    ...
    for il_num in range(section_edge[0], section_edge[1] + 1):
        ...
        for xl_num in range(section_edge[2], section_edge[3] + 1):
            ...
            for z_num in range(section_edge[5] - section_edge[4] + 1):
                ...
                data[indx * (section_edge[5] - section_edge[4] + 1) + z_num, :, :, :, :] = \
                    cube_parse(seis_arr=inp_seis,
                               cube_incr=cube_incr,
                               inp_res=inp_res,
                               mode='point',
                               padding=False,
        ...
        with segyio.open(output_file, "r+") as src:
            ...
            for ilno in src.ilines:
                ...
                if src.ilines[section_edge[0]] <= ilno <= src.ilines[section_edge[1]]:
                    line = src.iline[ilno]
                    line[section_edge[2]:section_edge[3] + 1, section_edge[4]:section_edge[5] + 1] = prediction[i, :, :,
                                                                                                     class_row]
                    src.iline[ilno] = line
                    i += 1
